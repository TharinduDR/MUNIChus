...
def generate_caption_aya_few_shot(image, news_content, language, few_shot_examples):
    """Generate caption using Aya Vision 8B with few-shot examples"""

    # Convert target image to URL format
    ...
    # Add text-based few-shot examples
    ...
    # Add the target task
    prompt_parts.append("\nNow, write a caption for a new image.")
    ...
    prompt_parts.append("- Follow the style of the examples provided")
...
def evaluate_language_few_shot(lang_code, dataset_name="tharindu/MUNIChus", num_samples=None, num_few_shot=3):
    """Evaluate Aya Vision model on a specific language with few-shot learning"""
    ...
    for i, example in enumerate(tqdm(test_data, desc=f"Generating {lang_code} (few-shot)")):
        try:
            ...
            generated_caption = generate_caption_aya_few_shot(
                example['image'],
                example['content'],
                lang_code,
                few_shot_examples
...
if __name__ == "__main__":
    ...
    print(f"Starting {NUM_FEW_SHOT}-shot evaluation for Aya Vision 8B")
    print("Using random few-shot example selection")
    print("=" * 80)
    ...
    if not all_results:
        ...
        print("\n" + "=" * 80)
        print(f"FINAL RESULTS SUMMARY - AYA VISION 8B ({NUM_FEW_SHOT}-shot learning)")
        print("=" * 80)
        print(results_df.to_string(index=False))

        output_prefix = f"aya_vision_{NUM_FEW_SHOT}shot"
