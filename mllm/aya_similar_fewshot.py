...
def generate_caption_aya_similarity_fewshot(image, news_content, language, few_shot_examples):
    """Generate caption using Aya Vision 8B with similarity-based few-shot examples"""
...
def evaluate_language_similarity_fewshot(lang_code, dataset_name="tharindu/MUNIChus",
    ...
    for i, example in enumerate(tqdm(test_data, desc=f"Generating {lang_code} (similarity-based)")):
        try:
            ...
            generated_caption = generate_caption_aya_similarity_fewshot(
                example['image'],
                example['content'],
                lang_code,
                few_shot_examples
...
if __name__ == "__main__":
    ...
    print(f"Starting {NUM_FEW_SHOT}-shot SIMILARITY-BASED evaluation for Aya Vision 8B")
    ...
    if not all_results:
        ...
        print("\n" + "=" * 80)
        print(f"FINAL RESULTS SUMMARY - AYA VISION 8B (Similarity-based {NUM_FEW_SHOT}-shot)")
        print("=" * 80)
        print(results_df.to_string(index=False))

        output_prefix = f"aya_vision_{NUM_FEW_SHOT}shot_similarity"
