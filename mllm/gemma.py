...
def generate_caption_gemma(image, news_content, language):
    """Generate caption using Gemma-3n-e4b"""
...
Caption in {language_names[language]}:"""
    ...
    # Format messages for Gemma-3n
    messages = [
        {
            "role": "system",
            "content": [{"type": "text", "text": "You are a helpful assistant specialized in writing news captions."}]
        },
        {
            "role": "user",
            "content": [
...
def evaluate_language(lang_code, dataset_name="tharindu/MUNIChus", num_samples=None):
    """Evaluate Gemma-3n model on a specific language with BLEU-4, CIDEr, and chrF"""
    ...
    for i, example in enumerate(tqdm(test_data, desc=f"Generating {lang_code}")):
        try:
            generated_caption = generate_caption_gemma(
                example['image'],
                example['content'],
                lang_code
            )
...
# Print summary
print("\n" + "=" * 80)
print("FINAL RESULTS SUMMARY - GEMMA-3N-E4B-IT")
print("=" * 80)
print(results_df.to_string(index=False))

# Save results
results_df.to_csv("gemma3n_evaluation_results.csv", index=False)
print("\n✓ Results saved to gemma3n_evaluation_results.csv")

# Save detailed predictions
with open("gemma3n_predictions.json", "w", encoding="utf-8") as f:
    json.dump({
        "predictions": all_predictions,
        "references": all_references
    }, f, ensure_ascii=False, indent=2)
print("✓ Predictions saved to gemma3n_predictions.json")
