...
def generate_caption_qwen3_thinking(image, news_content, language):
    """Generate caption using Qwen3-VL-8B-Thinking"""
...
def evaluate_language(lang_code, dataset_name="tharindu/MUNIChus", num_samples=None):
    """Evaluate Qwen3-VL-Thinking model on a specific language with BLEU-4, CIDEr, and chrF"""
    ...
    for i, example in enumerate(tqdm(test_data, desc=f"Generating {lang_code}")):
        try:
            generated_caption = generate_caption_qwen3_thinking(
                example['image'],
                example['content'],
                lang_code
            )
...
# Print summary
print("\n" + "=" * 80)
print("FINAL RESULTS SUMMARY - QWEN3-VL-8B-THINKING")
print("=" * 80)
print(results_df.to_string(index=False))

# Save results
results_df.to_csv("qwen3_thinking_evaluation_results.csv", index=False)
print("\n✓ Results saved to qwen3_thinking_evaluation_results.csv")

# Save detailed predictions
with open("qwen3_thinking_predictions.json", "w", encoding="utf-8") as f:
    json.dump({
        "predictions": all_predictions,
        "references": all_references
    }, f, ensure_ascii=False, indent=2)
print("✓ Predictions saved to qwen3_thinking_predictions.json")
