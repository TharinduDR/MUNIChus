...
def generate_caption_few_shot(image, news_content, language, few_shot_examples):
    """Generate caption using Llama-3.2-11B-Vision with few-shot examples"""
...
def evaluate_language_few_shot(lang_code, dataset_name="tharindu/MUNIChus", num_samples=None, num_few_shot=3):
    """Evaluate Llama Vision model on a specific language with few-shot learning"""
    ...
    for i, example in enumerate(tqdm(test_data, desc=f"Generating {lang_code} (few-shot)")):
        try:
            ...
            generated_caption = generate_caption_few_shot(
                example['image'],
                example['content'],
                lang_code,
                few_shot_examples
...
if __name__ == "__main__":
    ...
    print(f"Starting {NUM_FEW_SHOT}-shot evaluation for Llama-3.2-11B-Vision")
    ...
    if not all_results:
        ...
        print("\n" + "=" * 80)
        print(f"FINAL RESULTS SUMMARY - LLAMA 3.2 11B VISION ({NUM_FEW_SHOT}-shot learning)")
        print("=" * 80)
        print(results_df.to_string(index=False))

        output_prefix = f"llama_vision_{NUM_FEW_SHOT}shot"
