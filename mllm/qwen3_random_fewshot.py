...
def generate_caption_qwen3_few_shot(image, news_content, language, few_shot_examples):
    """Generate caption using Qwen3-VL-8B with few-shot examples"""

    # Build the message content with few-shot examples
    message_content = []
    ...
    message_content.append({
        "type": "text",
        "text": f"""
Given the news article excerpt:
{news_content[:1200]}
...
def evaluate_language_few_shot(lang_code, dataset_name="tharindu/MUNIChus", num_samples=None, num_few_shot=3):
    """Evaluate Qwen3-VL model on a specific language with few-shot learning"""

    print(f"\n{'=' * 80}")
    print(f"Evaluating {language_names[lang_code]} ({lang_code}) - {num_few_shot}-shot")
    print(f"{'=' * 80}")
    ...
    for i, example in enumerate(tqdm(test_data, desc=f"Generating {lang_code} ({num_few_shot}-shot)")):
        try:
            ...
            generated_caption = generate_caption_qwen3_few_shot(
                example['image'],
                example['content'],
                lang_code,
                few_shot_examples
...
if __name__ == "__main__":
    # Configuration
    NUM_FEW_SHOT = 3  # Number of few-shot examples to use (0, 1, 3, or 5)
    ...
    print(f"Starting {NUM_FEW_SHOT}-shot evaluation...")
    print("=" * 80)
    ...
    print("\n" + "=" * 80)
    print(f"FINAL RESULTS SUMMARY - QWEN3-VL-8B-INSTRUCT ({NUM_FEW_SHOT}-shot learning)")
    print("=" * 80)
    print(results_df.to_string(index=False))

    output_prefix = f"qwen3_vl_{NUM_FEW_SHOT}shot"
