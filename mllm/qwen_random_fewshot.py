...
def generate_caption_qwen_few_shot(image, news_content, language, few_shot_examples):
    """Generate caption using Qwen2.5-VL with few-shot examples"""
    ...
    for i, example in enumerate(few_shot_examples, 1):
        ...
        message_content.append({
            "type": "text",
            "text": f"News excerpt: {example['content']}\nCaption in {language_names[language]}: {example['caption']}\n"
        })

    # Add the target task
    message_content.append({
        "type": "text",
        "text": f"\nNow, write a caption for this new image following the same style:"
    })
    message_content.append({
        "type": "image",
        "image": image
...
def evaluate_language_few_shot(lang_code, dataset_name="tharindu/MUNIChus", num_samples=None, num_few_shot=3):
    """Evaluate Qwen2.5-VL model on a specific language with few-shot learning"""
    ...
    for i, example in enumerate(tqdm(test_data, desc=f"Generating {lang_code} (few-shot)")):
        try:
            ...
            if len(few_shot_examples) < num_few_shot:
                print(f"Warning: Only {len(few_shot_examples)} examples available")

            # Generate caption with few-shot examples
            generated_caption = generate_caption_qwen_few_shot(
                example['image'],
                example['content'],
                lang_code,
                few_shot_examples
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print(f"FINAL RESULTS SUMMARY - QWEN2.5-VL-7B ({NUM_FEW_SHOT}-shot learning)")
    print("=" * 80)
    print(results_df.to_string(index=False))

    output_prefix = f"qwen2_vl_{NUM_FEW_SHOT}shot"
